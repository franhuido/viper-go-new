...
import { X, Settings, Clock, RotateCcw, LogOut } from 'lucide-react';

interface SidebarMenuProps {
  isOpen: boolean;
  onClose: () => void;
  onDiagnostic: () => void;
  onCommunicationHistory: () => void;
  onForceVertical: () => void;
  onLogout: () => void;
}
      ...
      >
        ...
        <div className="py-6">
          <button
            ...
            className="flex items-center gap-4 w-full px-6 py-4 hover:bg-gray-50 transition-colors text-left"
          >
            <Settings size={24} className="text-gray-600" />
            <span className="text-lg text-gray-700">Diagnóstico</span>
          </button>

          <button
            onClick={onCommunicationHistory}
            className="flex items-center gap-4 w-full px-6 py-4 hover:bg-gray-50 transition-colors text-left"
          >
            <Clock size={24} className="text-gray-600" />
            <span className="text-lg text-gray-700">Historial de Comunicaciones</span>
          </button>

          <button
            onClick={onForceVertical}
            className="flex items-center gap-4 w-full px-6 py-4 hover:bg-gray-50 transition-colors text-left"
          >
            <RotateCcw size={24} className="text-gray-600" />
            <span className="text-lg text-gray-700">Forzar vista horizontal</span>
          </button>
